...
class TestPrompts:
    ...
    def test_generation_prompt_contains_voice_identity(self):
        ...
        for element in voice_elements:
            assert element.lower() in prompt.lower(), f"Missing '{element}' in generation prompt"
...
class TestAudioHelpers:
    ...
    def test_generate_room_tone(self):
        ...
        from generate_audio import generate_room_tone, AudioConfig

        config = AudioConfig()
        room_tone = generate_room_tone(config)

        # Should produce 0.5 seconds of 24kHz 16-bit mono audio
        expected_bytes = int(config.sample_rate * config.room_tone_duration * config.sample_width)
        assert len(room_tone) == expected_bytes

    def test_pcm_to_wav(self):
        """Verify PCM to WAV conversion works."""
        import wave
        from generate_audio import pcm_to_wav, generate_room_tone, AudioConfig
...
class TestGeminiIntegration:
    ...
    def test_import_dependencies(self):
        ...
        from generate_audio import genai, AudioSegment, whisper
    ...
    async def test_beat_sheet_generation(self, sample_report):
        ...
        from generate_audio import import_dependencies, generate_beat_sheet, AudioConfig
        import_dependencies()
    ...
    async def test_short_audio_generation(self):
        ...
        from generate_audio import import_dependencies, generate_audio_segment, AudioConfig
        import_dependencies()
