...
def run_perplexity_research(
    ...
    for attempt in range(max_retries):
        try:
            ...
            response = requests.post(url=SYNC_URL, json=payload, headers=headers, timeout=timeout)
            break
...
Environment:
    ...
    mode_group.add_argument(
        '--sync', dest='use_sync', action='store_true',
    ...
    parser.add_argument(
        ...
        help='List all async jobs for this API key'
    )
    parser.add_argument(
    ...
    parser.add_argument('--show-cost', action='store_true', help='Display cost breakdown')
    parser.add_argument('--auto-save', action='store_true',
                        help='Automatically save output with timestamp (default when no --output)')
    parser.add_argument('--no-auto-save', action='store_true', help='Disable automatic file saving')
    parser.add_argument('--log-dir', help='Directory for output and log files (default: current directory)')
    ...
    # Determine mode
    is_async = args.use_async or args.no_wait or args.job_id is not None or args.list_jobs
    ...
    if args.list_jobs:
        ...
        for job in jobs:
            ...
            print(f"{job.get('id', 'N/A'):<40} {job.get('status', 'N/A'):<14} {job.get('model', 'N/A'):<25} {created}")
        sys.exit(0)
    ...
    if is_async:
        ...
        if args.no_wait:
            print(f"\nJob ID: {job_id}")
            print(f"Poll later with: python {sys.argv[0]} --job-id {job_id} --output results.md")
            sys.exit(0)
