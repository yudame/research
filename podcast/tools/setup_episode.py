...
def create_brief_template(date: str, title: str, context: str = None) -> str:
    ...
    context_section = context if context else "[High-level description of what this episode will research]"
...
def get_episode_path(base_dir: Path, date: str, slug: str, series: str = None, episode_num: int = None) -> Path:
    ...
    if series:
        ...
        return base_dir / "podcast" / "episodes" / series / ep_dir
    else:
        # Standalone episode: YYYY-MM-DD-slug/
        return base_dir / "podcast" / "episodes" / f"{date}-{slug}"
...
"""
    ...
    parser.add_argument(
        "--title", "-t",
        required=True,
        help="Full episode title"
    )
    parser.add_argument(
        "--series",
        help="Series directory name (e.g., 'cardiovascular-health-series')"
    )
    parser.add_argument(
        "--episode-num", "-n",
        type=int,
        help="Episode number within series"
    )
    parser.add_argument(
        "--context", "-c",
        help="Research context or focus for p1-brief.md"
    )
    parser.add_argument(
        "--quiet", "-q",
        action="store_true",
