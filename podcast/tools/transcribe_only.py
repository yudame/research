...
def transcribe_audio(audio_path: str, model_name: str = "base", verbose: bool = True, log_file: str = None):
...
def transcribe_audio_api(audio_path: str, model_name: str = "whisper-1", verbose: bool = True, log_file: str = None):
...
def main():
    ...
    if args.use_api:
        result = transcribe_audio_api(str(audio_path), verbose=not args.quiet, log_file=log_file)
        if result is None:
            return 1
    else:
        result = transcribe_audio(str(audio_path), args.model, verbose=not args.quiet, log_file=log_file)
