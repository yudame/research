...
Usage:
    python update_feed.py ../episodes/2025-12-26-topic-slug/
    python update_feed.py ../episodes/cardiovascular-health/ep6-lifestyle/
    python update_feed.py ../episodes/2025-12-26-topic-slug/ --dry-run
...
def parse_metadata_md(metadata_path: Path) -> dict:
    ...
    # Extract key sources
    sources_match = re.search(r'^## Key Sources.*?\n(.+?)(?=\n#|\Z)', content, re.MULTILINE | re.DOTALL)
    if sources_match:
        ...
        for line in sources_text.split('\n'):
            ...
            if ':' in line and 'http' in line:
                ...
                if line.startswith('['):
                    ...
                    if link_match:
                        sources.append({'name': link_match.group(1), 'url': link_match.group(2)})
                else:
                    # "Name: URL" format
                    parts = line.split(':', 1)
                    if len(parts) == 2:
...
def build_episode_url(episode_dir: Path, filename: str) -> str:
    ...
    # Get path relative to podcast/episodes/
    try:
...
def generate_item_xml(metadata: dict, episode_dir: Path, audio_file: Path) -> str:
    ...
    if 'keywords' in metadata:
        lines.append(f"      <itunes:keywords>{escape(metadata['keywords'])}</itunes:keywords>")

    if 'series_name' in metadata:
        lines.append(f"      <research:series>{escape(metadata['series_name'])}</research:series>")
...
def update_feed_xml(feed_path: Path, item_xml: str, episode_title: str) -> str:
    ...
    if first_item_match:
        ...
        short_title = episode_title.split(':')[0] if ':' in episode_title else episode_title
        episode_comment = f"\n\n    <!-- Episode: {short_title} -->\n"

        new_content = content[:insert_pos] + episode_comment + item_xml + content[insert_pos:]
    else:
        ...
        new_content = content[:insert_pos] + episode_comment + item_xml + "\n\n  " + content[insert_pos:]
...
Examples:
  ...
  %(prog)s ../episodes/2025-12-26-topic-slug/

  # Add series episode
  %(prog)s ../episodes/cardiovascular-health/ep6-lifestyle/

  # Preview without writing
  %(prog)s ../episodes/2025-12-26-topic-slug/ --dry-run
"""
    )
