...
def add_branding(cover_path, series_text=None, episode_text=None,
    ...
    log(f"Background brightness: {brightness:.0f} ({'light' if is_light_bg else 'dark'})")
    ...
    if series_text:
        if use_shadow:
            draw.text((current_x + shadow_offset, current_y + shadow_offset),
                      series_text, fill=(0, 0, 0, 100), font=series_font)
        draw.text((current_x, current_y), series_text, fill=text_secondary, font=series_font)
